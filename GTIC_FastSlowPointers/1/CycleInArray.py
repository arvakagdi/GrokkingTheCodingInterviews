...
if ‘M’ is positive we will move forward ‘M’ indices and if ‘M’ is negative move backwards ‘M’ indices. You should assume that the array is 
...
def circular_array_loop_exists(arr):
  for i in range(len(arr)):
    ...
    while True:
      slow = next_index(arr, is_forward, slow)
      fast = next_index(arr, is_forward, fast)
...
def next_index(arr, is_forward, curr_index):
